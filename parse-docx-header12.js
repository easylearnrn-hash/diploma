    ...
    if (fileName === 'word/header1.xml') {
      entry.buffer().then(content => {
        ...
        for (const line of lines) {
           ...
           if (inTable && line.includes('ACADEMIC')) pPrXml.push(line);
           if (inTable && line.includes('jc w:val')) pPrXml.push(line);
           if (inTable && line.includes('<w:t xml:space')) pPrXml.push(line);
        }
        console.log(pPrXml);
      });
    } else {
